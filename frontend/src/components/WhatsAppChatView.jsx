...
export default function WhatsAppChatView({ token }) {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      const res = await API('/api/conversations?limit=100', token);
  ...
  const fetchMessages = useCallback(
    async (conversationId) => {
      try {
        const res = await API(`/api/messages?conversationId=${conversationId}&limit=200`, token);
        const data = await res.json();
        if (res.ok && Array.isArray(data.data)) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => fetchMessages(selected.id), 10000);
    return () => clearInterval(timer);
  ...
  const openConversation = async (conv) => {
    ...
    if (conv.unreadCount > 0) {
      try {
        await API(`/api/conversations/${conv.id}/read`, token, { method: 'POST' });
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await API('/api/messages/send', token, {
        method: 'POST',
